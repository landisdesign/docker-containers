...
RC=0
awk -v apos="'" 'NR==FNR {a[$1]=""} NR!=FNR {if ($1 in a) a[$1]=($2 " " a[$1])} END {g="";x="";for (i in a) {if (a[i]) g=(g i " " a[i] "\n"); else x=(apos i apos " " x);} if (x) {print "No secrets held the following data: " x | "cat 1>&2"; exit 1} else printf("%s", g);}' ./~search_data.txt ./~secret_data.txt || RC=$?

rm -f ./~search_data.txt ./~secret_data.txt
